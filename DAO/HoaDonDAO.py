...
class HoaDonDAO:
    ...
    def get_customer_by_id(self, id):
        """Lấy thông tin hóa đơn theo ID"""
        query = "SELECT * FROM bills, customers WHERE bills.id = %s and bills.customer_id = customers.id"
        row = self.db.fetch_all(query, (id,))
        if row:
            return row[0]["name"]
        return None
    def get_employee_by_id(self, id):
        """Lấy thông tin hóa đơn theo ID"""
        query = "SELECT * FROM bills, employees WHERE bills.id = %s and bills.processed_by = employees.id"
        row = self.db.fetch_all(query, (id,))
        if row:
            return row[0]["name"]
        return None
    ...
    def insert_invoice(self, customer_id, meter_id, reading_id, amount, totalBill, status, processed_by):
        ...
        """
        success = self.db.execute_query(query, (customer_id, meter_id, reading_id, amount, totalBill, status, today, processed_by))
