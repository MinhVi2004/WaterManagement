...
class HoaDonDAO:
    ...
    def search_invoice(self, keyword, search_type):
      if keyword == "":
            # Nếu không có keyword, trả về tất cả hóa đơn
            return self.get_all()

      """Tìm kiếm hóa đơn theo ID, customer_id, hoặc meter_id"""
      if search_type == "Mã Hóa Đơn":
            query = "SELECT * FROM bills WHERE id = %s"
            param = (keyword,)  # Truyền chính xác mã hóa đơn
      elif search_type == "Tên Khách Hàng":
            query = "SELECT b.* FROM bills b JOIN customers c ON c.id = b.customer_id WHERE c.name LIKE %s"
            param = ('%' + keyword + '%',)  # Tìm kiếm gần đúng theo tên khách hàng
      elif search_type == "Tên Nhân Viên":
            query = "SELECT b.* FROM bills b JOIN employees e ON e.id = b.processed_by WHERE e.name LIKE %s"
            param = ('%' + keyword + '%',)  # Tìm kiếm gần đúng theo tên nhân viên
      elif search_type == "Trạng Thái":
            query = "SELECT b.* FROM bills b WHERE status = %s"
            param = (keyword,)  # Tìm kiếm chính xác theo trạng thái
      else:
            raise ValueError("Invalid search type")

      rows = self.db.fetch_all(query, param)
      listHD = []
      for row in rows:
            hoa_don_dto = HoaDonDTO(
                  row['id'], row['customer_id'], row['meter_id'], row['reading_id'], 
                  row['amount'], row['totalBill'], row['status'], row['created_at'], row['processed_by']
            )
            listHD.append(hoa_don_dto)
